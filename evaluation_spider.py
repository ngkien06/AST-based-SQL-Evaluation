...
def evaluate(gold, predict, db_dir, etype, kmaps):
    ...
    for p, g in zip(plist, glist):
        ...
        try:
            ...
            #print("Error in parsing predicted SQL: {}".format(p_str))
        ...
        #print(f"Evaluating SQL {curr_idx}")
        if etype in ["all", "exec"]:
            ...
            if exec_score:
                ...
                print(f"incorrect exec count: {len(incorrect_exec_sql)}")                                   
                #print("pred: {}".format(p_str))
                incorrect_exec_sql.append({
                    'predictSQL': p_str,
                    'goldSQL': g_str,
    ...
    # extract to a file for error analysis
    print(f"Total incorrect: {len(incorrect_exec_sql)}")
    
    with open("spider_test_incorrect_sql_CodeS.json", "w") as f:
        json.dump(incorrect_exec_sql, f, indent=4)
    '''
    with open("spider_test_gold_ast.json", "w") as f:
        json.dump(gold_sql_ast, f, indent=4)
    '''
...
if __name__ == "__main__":
    ...
    print(f"gold: {gold}, pred: {pred}, db: {db_dir}, etype: {etype}")
